...
describe('access-denied-controller', () => {
  const createReqRes = () => {
  ...
  it('calls initialiseBasicAuthentication', async () => {
    ...
    await accessDeniedController(req, res, next)
    expect(mockInitialiseBasicAuthentication).toHaveBeenCalledWith(req, res, next)
  })

  it('sets pageTitle and backLink then renders access-denied', async () => {
    const { req, res, next } = createReqRes()
    await accessDeniedController(req, res, next)
    expect(res.locals.pageTitle).toBe('Access denied')
    expect(res.locals.backLink).toBe(paths.START)
    expect(res.render).toHaveBeenCalledWith('pages/access-denied.njk')
  })
